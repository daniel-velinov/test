    ...
    steps {
      script {
		 ...
		 // sh "echo '${propertyFile}' > propertyFile-$BUILD_NUMBER.js"
		  sh "cd /tmp && echo '${propertyFile}' > propertyFile-$BUILD_NUMBER.js && ansible-playbook -i hosts.ini test.yml --extra-vars 'count=${numContainers} build=$BUILD_NUMBER state=${reportState}'"
        }
      }
    }
  }
